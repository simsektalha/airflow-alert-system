...
def build_agent_team_from_cfg(cfg: Dict[str, Any]) -> Optional["Team"]:
    """Build Agent Teams for enhanced failure analysis."""
    if Agent is None or Team is None:
        LOG.info("[llm] agno not installed; skipping Agent Teams")
        return None
    ...
    if driver == "openai_like" and OpenAILike and llm.get("base_url") and llm.get("api_key"):
        ...
        LOG.info("[llm] not configured or driver unavailable; skipping Agent Teams")
        return None
    ...
    LOG.info("[llm] Agent Teams initialized successfully")
    return team


def build_agent_from_cfg(cfg: Dict[str, Any]) -> Optional["Agent"]:
    """Build single agent (legacy fallback)."""
    if Agent is None:
...
async def ask_agent_team_for_analysis(team: "Team", error_focus: str, log_tail: str, identifiers: Dict[str, Any]) -> Dict[str, Any]:
    """Ask Agent Teams for collaborative analysis."""
    prompt = f"""
# Airflow Failure: Team Collaboration
DAG: {identifiers.get('dag_id')}
Run: {identifiers.get('dag_run_id')}
Task: {identifiers.get('task_id')}
Try: {identifiers.get('try_number')}
...
"""
    ...
    LOG.info("[llm] Agent Teams response received size=%d", len(text))
    
    # Parse the response to extract JSON-like structure
    # For now, create a structured response based on the team output
    return {
        "root_cause": text[:500] if text else "Unknown",
        "category": "other",
        "fix_steps": ["Review Agent Teams analysis", "Apply suggested fixes", "Monitor results"],
        "prevention": ["Use Agent Teams for future analysis", "Monitor system health"],
        "needs_rerun": True,
        "confidence": 0.8,
        "error_summary": text[:200] if text else "Agent Teams analysis completed",
    }
...
# 3) Analyze with Agent Teams (or fallback to single agent)
    team = build_agent_team_from_cfg(cfg)
    agent = build_agent_from_cfg(cfg)
    
    LOG.info("[llm] Agent Teams available=%s, Single Agent available=%s", bool(team), bool(agent))

    if team is not None:
        try:
            # Use Agent Teams for enhanced analysis
            LOG.info("[llm] Using Agent Teams for analysis")
            analysis = await ask_agent_team_for_analysis(
                team=team,
            ...
            LOG.warning("[llm] Agent Teams analysis failed (%s); fallback to single agent", e)
