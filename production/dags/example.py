...
# Import Agent Teams for DAG-level callbacks
from src.event_driven.callbacks import create_dag_failure_callback
from src.event_driven.config import AlertConfig

LOG = logging.getLogger("realistic-failures")

# Create Agent Teams DAG-level callback
try:
    config = AlertConfig()
    config.validate()
    dag_failure_callback = create_dag_failure_callback(config)
    LOG.info("Agent Teams DAG callback initialized successfully")
except Exception as e:
    LOG.warning(f"Failed to initialize Agent Teams DAG callback: {e}")
    dag_failure_callback = None

default_args = {
    "on_failure_callback": on_failure_trigger_fetcher,  # Task-level callback (enhanced)
}
...
with DAG(
    ...
    on_failure_callback=dag_failure_callback,  # DAG-level callback (Agent Teams)
    description="Enhanced failure analysis with Agent Teams + legacy fallback",
) as dag:
